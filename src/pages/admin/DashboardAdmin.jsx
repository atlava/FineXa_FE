  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
        ...
        {/* KARTU 1: Total Users (Lebih Clean Tanpa Persentase) */}
        <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
          ...
          <span className="text-2xl md:text-3xl font-bold text-gray-800 mt-1">{stats.total_users || 0}</span>
        </div>

        {/* KARTU 2: ACTIVE USERS / MONTH */}
        <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
          <div className="flex justify-between items-start">
            <span className="p-2.5 bg-gray-50 rounded-xl text-lg md:text-xl">📈</span>
            <span className={`text-xs font-bold flex items-center gap-1 ${stats.active_growth >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              {stats.active_growth >= 0 ? '↗ +' : '↘ '}{stats.active_growth || 0}%
            </span>
          </div>
          <span className="text-gray-400 text-xs font-medium mt-4">Login Users / Month</span>
          <span className="text-2xl md:text-3xl font-bold text-gray-800 mt-1">{stats.active_sessions || 0}</span>
        </div>

        {/* KARTU 3: Content Items (FAQ) (Lebih Clean Tanpa Persentase) */}
        <div className="bg-white p-5 md:p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:col-span-2 md:col-span-1">
          <div className="flex items-start">
            <span className="p-2.5 bg-gray-50 rounded-xl text-lg md:text-xl">✏️</span>
          </div>
          <span className="text-gray-400 text-xs font-medium mt-4">Content Items</span>
          <span className="text-2xl md:text-3xl font-bold text-gray-800 mt-1">{stats.total_faq || 0}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        {/* Kanan: Distribusi Profil Risiko */}
